({
    ...
    loadAssetOnMap: function(component, event, helper) {
        ...
        if(country == "Mexico"){
            //map.panTo(new L.LatLng(23.6345, -102.5528));
             //map.setView([latitude, longitude], 6);
             map.setView(new L.LatLng(23.6345, -102.5528), 6.5); 
        ...
        var loadedAssets = 0;
        const Language = component.get("v.Language");
    	console.log('line 109--English' + Language);
        ...
        Object.values(groupedAssets).forEach(function(accgroup) {
            ...
            if(Language == 'es'){
                 popupContent = 
                'Cuenta: ' + accgroup.Account + '<br>' +
                'Número de activos: ' + accgroup.count + '<br>' +
                rackTypeCounts;
            }
            else{
                  popupContent = 
        ...
        function addMarkersBatch(startIndex) {
            ...
            for (var i = startIndex; i < endIndex; i++) {
                var asset = inTransitAssets[i];
                 var stateofAsset = asset.State_of_Pallet__c;
                if(asset.State_of_Pallet__c  == "In Transit"){
                    stateofAsset = $A.get("$Label.c.In_Transit");
                }else if (asset.State_of_Pallet__c  == "Lost"){
                    stateofAsset = "Perdida";
                }else{
                    stateofAsset = "En red";
                }
                var undefCustomer = asset.Field2__c;
                if(asset.Field2__c == null || asset.Field2__c == "" || asset.Field2__c == undefined){
                    undefCustomer = "indefinida";
                }
                var undefAssetTyepe = asset.Rack_Type__c;
                 if(asset.Rack_Type__c == null || asset.Rack_Type__c == "" || asset.Rack_Type__c == undefined){
                    undefAssetTyepe = "indefinida";
                }
                ...
                if (newId == 'Customer User' && asset.Total_Dwell_Days__c >= asset.Rogue_Asset_Day_Limit__c && asset.Rogue_Asset_Day_Limit__c > 0) {
                    var popupContent;
                    	 if(Language == 'es'){
                          ...
                          popupContent = 'Nombre del activo: ' + asset.Asset_Name_URL__c + '<br>' +
                                 'ID de cliente: ' + undefCustomer + '<br>' +
                                 'Tipo de activo: ' + undefAssetTyepe + '<br>' +
                                 'Último conectado: ' + formattedDate + '<br>' +
                                 'Estado del activo: ' + stateofAsset + '<br>' +
                                 'Dirección de ubicación actual: ' + asset.Current_Location_Address__c + '<br>' +
                                 'Pistas INTELIGENTES: ' + asset.Smart_Tracks2__c;
                         }else{
                              popupContent =  'Asset Name: ' + asset.Asset_Name_URL__c + '<br>' +
